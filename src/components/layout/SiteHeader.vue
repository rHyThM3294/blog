...
watch(() => props.isOpen, (open) => {
  if (open) {
    // bars 消失
    gsap.to(barsRef.value, { opacity: 0, rotate: 90, scale: 0.5, duration: 0.25, ease: 'power2.in' })
    // xmark 出現
    gsap.fromTo(
      xmarkRef.value,
    ...
    // xmark 消失
    gsap.to(xmarkRef.value, { opacity: 0, rotate: 90, scale: 0.5, duration: 0.25, ease: 'power2.in' })
    // bars 出現
    gsap.fromTo(
      barsRef.value,
      { opacity: 0, rotate: -90, scale: 0.5 },
...
function toggleMenu(){ emit('update:isOpen', !props.isOpen) }
function closeMenu(){ emit('update:isOpen', false) }
function handleResize(){ if (window.innerWidth > 768) closeMenu() }
...
.router-link-active.navLink { color: var(--mainColor); }
...
@media(width > 768px){
  ...
  .navLink:hover{ color: var(--mainColor); }
}
</style>