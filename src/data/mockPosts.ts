...
export const mockPosts: Post[] = [
  {
    ...
    content: `
      ...
      「地圖 × 產季」這個頁面是研究過程中蠻有趣的一個結果。訪談中不少人提到「不知道什麼季節買什麼比較好」，這讓我們決定把台灣地圖和產季資訊整合在同一個頁面，讓使用者可以先選縣市，看到當地的代表農產品，或是透過產季視圖，知道現在適合買什麼。
...
這個專案有 12 個前台頁面和 2 個後台管理頁面，元件數量不少。如果每個頁面都從頭寫，程式碼重複率會很高；但如果拆得太細，每個元件的 Props 介面變得複雜，呼叫端反而變難維護。
...
地圖頁有三個子功能：台灣地圖、產季圖表、季節商品列表。這些資料彼此有關聯（點選縣市要更新商品列表、切換季節要更新圖表），如果把資料散在各個元件裡，父子元件之間的 emit/props 傳遞會很混亂。
...
後台管理頁面（AdminDashboard）裡，我一開始把「搜尋欄」、「日期篩選」、「狀態篩選」各自拆成獨立元件，結果父元件要用 \`v-model\` 接三組不同的資料，呼叫端變得很囉嗦。
